...
class TotalOrderingChannel:
        ...
        while True:
            if self.reliable_channel.can_recv():
                ...
                if isinstance(msg, MulticastMessage):
                    ...
                    # means that someone responded. put it on the list of
                    # responses for the id
                    self.response_dict[msg.msg_id].append(msg)
                    # print self.response_dict[msg.msg_id]
                    # need to account for number of processes in group which
                    # was initialized on the multicast
                    if len(self.response_dict[msg.msg_id]) == self.group_size_dict[msg.msg_id]:
                        ...
                        final_msg = MarkedMessage(
                            final_priority.seq, self.msg_dict[msg.msg_id], msg.msg_id, self.addr)
                        # recreate the group of processes to send a final
                        # priority
                        group = [
                            proposal.addr for proposal in list_of_responses]
                        # print str(self.from_id) + " " +
                        # str(self.msg_dict[msg.msg_id]) +  " TO GROUP" +
                        # str(group)
                    ...
                    # remove from undeliverable list and put in deliverable
                    # priority queue
    ...
    def multicast(self, obj, group, from_id):
        # verified that group size is correct. need to create a mapping between
        # msg_id and group size
