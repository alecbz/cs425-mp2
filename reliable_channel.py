...
class ReliableChannel:
        ...
        while True:
            ...
            #message isn't an Ack
                ...
                with self.messages_cond:
                    #if the msg sequence isn't earlier than next seq to pop, store it. fifo ordering
                    if self.ordering_scheme == "fifo_ordering":
    ...
    def _available(self):
        #need to deepcopy so that size of dictionary does not change when iterating
        if self.ordering_scheme == "fifo_ordering":
            messages_copy = copy.deepcopy(dict(self.messages))
            retlist =  [(addr, heap)
                    for (addr, heap) in messages_copy.iteritems()
                    #if heap is non-empty and msg seq number equals the next seq number to be read
                    ...
                    for i in range(0,len(self.msg_vector)):
                        if i != msg.data.id:
                            if self.msg_vector[i] < msg.data.vector[i]:
                                causal_cond = False
                                break
                    fifo_cond = (msg.data.vector[msg.id] == (self.msg_vector[msg.id]+1))
    ...
    def unicast_causal(self, data, addr, from_id):
        ...
        while True: 
            ...
            with self.acks_cond:
                ...
                start = time.time()
                vector_str = self.intlist_to_string(msg.data.vector)
                #if addr and seq number aren't in acks set, wait until they are 
                while not ((addr, vector_str) in self.acks) and (
                        time.time() - start < max_wait):
                    self.acks_cond.wait(0.1)
                #if ack has been received for a message to a addr, remove the tuple
